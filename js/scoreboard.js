...
function init(){
	...
	function scoreboard(){
		...
		if(startup == true){
			...
			if(game == 'BBTAG' || game == 'SFVAE' || game == 'TEKKEN7'){
			...
			if($('#gameHold').text() != game){ //checks to see if current json value for 'game' has changed from what is stored in gameHold html object
				...
				TweenMax.to('.logos',.3,{css:{opacity: 0},delay:0,onComplete:function(){ //then execute function
					...
					if(game == 'BBTAG' || game == 'SFVAE' || game == 'TEKKEN7'){
						...
						TweenMax.set('#rdWrapper',{css:{y: '0px'}});
						TweenMax.set('#p2Score',{css:{x: '0px'}});
						TweenMax.set('#p1Score',{css:{y: '0px'}});
						TweenMax.set('#p2Score',{css:{y: '0px'}});
					 ...
					 if(game == 'UNIST' || game == 'MELTY'){
						...
						TweenMax.set('#rightWrapper',{css:{y: '+0px'}});
						TweenMax.set('#leftWrapper',{css:{x: '0px'}}); //same functions as above but this time also return wrappers to original positioning
						TweenMax.set('#rightWrapper',{css:{x: '-90px'}});
						TweenMax.set('#rdWrapper',{css:{y: '+25px'}});
						TweenMax.set('#p2Score',{css:{x: '90px'}});
					}
